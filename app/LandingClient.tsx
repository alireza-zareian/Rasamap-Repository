...
/** How long the featured gallery holds each card before sliding on. */
const GALLERY_MS = 4000;

/**
 * The landing page's markup and its browser-side behaviour.
 *
 * Split out of page.tsx in V1 so the statistics and the featured media could be
 * read from the database on the server and passed in as props. Everything that
 * is left here needs a browser: the theme toggle, the sign-in state, the scroll
 * threshold behind the header, and the gallery's auto-advance.
 */
export default function LandingClient({
  ...
  useEffect(() => {
    ...
    autoRef.current = setInterval(() => setGalIdx(i => (i + 1) % billboards.length), GALLERY_MS);
    return () => { if (autoRef.current) clearInterval(autoRef.current); };
  }, [billboards.length]);

  const goGal = (dir: 1 | -1) => {
    if (autoRef.current) clearInterval(autoRef.current);
    setGalIdx(i => (i + dir + billboards.length) % billboards.length);
    autoRef.current = setInterval(() => setGalIdx(i => (i + 1) % billboards.length), GALLERY_MS);
  };
  ...
  return (
    <div style={{ fontFamily: "Vazirmatn Variable, Vazirmatn, sans-serif", direction: "rtl", color: "var(--text-main)", background: "var(--bg-deep)", minHeight: "100vh" }}>
      ...
      {/* ── Featured billboard gallery — omitted entirely if nothing in the
             catalogue has a photograph, rather than shown as an empty frame ── */}
      {billboards.length > 0 && (
          ...
          <div style={{ position: "relative" }}>
            ...
            <div style={{ overflow: "hidden", borderRadius: 16 }}>
              <div style={{ display: "flex", gap: 16, transform: `translateX(calc(${galIdx} * (280px + 16px)))`, transition: "transform 0.55s cubic-bezier(0.4, 0, 0.2, 1)" }}>
                {[...billboards, ...billboards.slice(0, 4)].map((b, i) => (
                  <Link
                    key={i}
      ...
      <section style={{ padding: "56px 28px", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", background: "var(--bg-card)" }}>
        <div className="stats-bar-grid" style={{ maxWidth: 900, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 24, textAlign: "center" }}>
          {[
            { num: faNum(stats.total) + "+", label: "رسانه ثبت‌شده", Icon: Megaphone, color: "var(--accent)" },
            { num: faNum(Math.round(stats.totalDailyReach / 1_000_000)) + "M+", label: "تردد روزانه بازار", Icon: Eye, color: "var(--green-accent)" },
            { num: faNum(stats.cityCount), label: "شهر پوشش‌داده", Icon: Building2, color: "var(--accent-warm)" },
            ...
            {types.map(t => (
              <Link key={t.type} href={`/explore?type=${t.type}`} style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 14, padding: "24px", textDecoration: "none", color: "var(--text-main)", display: "flex", alignItems: "center", gap: 16 }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>{faNum(stats.byType[t.type] ?? 0)} رسانه موجود</div>
                </div>
      ...
      <section className="section-halo" style={{ padding: "100px 28px", textAlign: "center" }}>
        ...
        <p style={{ color: "var(--text-muted)", marginBottom: 36, fontSize: "0.95rem" }}>بیش از {faNum(stats.total)} رسانه منتظرته — رایگان شروع کن</p>
