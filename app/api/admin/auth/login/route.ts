...
export const POST = defineRoute(
  {
    ...
    access: "public",
    // This account, then this address — see credentialAttempt in lib/rate-limit.
    rateLimit: { afterBody: (b, ip) => adminLoginAttempt(b.email, ip) },
    body: z.object({
  ...
  async ({ req, ip, userAgent, body }) => {
    ...
    await resetAccountAttempts("login", body.email);
