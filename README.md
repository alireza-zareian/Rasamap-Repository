# رساماپ — سامانه‌ی یافتن و رزرو رسانه‌های تبلیغاتی محیطی

رساماپ یک بازارگاهِ آنلاین برای بیلبورد و رسانه‌های محیطی است. کاربر بین چند هزار
رسانه جست‌وجو و فیلتر می‌کند، صفحه‌ی جزئیات را می‌بیند، و پس از ثبت‌نام با شماره‌ی
موبایل یک بازه‌ی زمانی را رزرو می‌کند. یک پنلِ مدیریتِ جداگانه هم برای رسیدگی به
رسانه‌ها و رزروها وجود دارد. رابطِ کاربری فارسی و راست‌به‌چپ است.
...
نمی‌ماند: اسکریپر رکوردِ تازه می‌آورد، مدیر ویرایش و حذف می‌کند، کاربر رزرو
می‌سازد. یک فایلِ کد را نمی‌شود نوشت. پس به یک پایگاه داده‌ی واقعی رفتیم. امروز
...
- **خواندن‌محور است.** کاتالوگی با حدود ۳۵۰۰ رکورد که مدام فهرست و فیلتر می‌شود.
  ...
  کار می‌کنند.
- **یک نقطه‌ی نوشتنِ تراکنشی دارد.** تنها نوشتنِ حساس، ساختِ رزرو است که کم پیش
  می‌آید و داخلِ یک تراکنش سریال می‌شود.
...
زیرِ بارِ سنگینِ نوشتن است — قفلِ تک‌نویسنده روی `POST /api/reservations` — و این را
در `docs/architecture.md` صریح گفته‌ایم.
...
- رزرو: بررسیِ هم‌پوشانی و درج داخلِ یک تراکنش؛ آزمونِ هم‌زمانی: ده درخواستِ یکسان
  و هم‌زمان، دقیقاً یک رزرو می‌سازد.
