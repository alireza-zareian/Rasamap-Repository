...
- `docs/engineering-decisions.md` — 23 decision records + milestone log (the "what we built and why" spine; §7a = why no Docker/ELK yet, §16 = SMS built-but-dormant, §17 = why there is no booking flow, §18 = monetisation without a gateway, §19 = upload hardening, §20 = anti-scraping, §21 = denormalised sort keys, §22 = why `npm run demo` and not `npm run dev`, §23 = the CRM question and the lead table built instead)
