...
def _layer(path):
    ...
    if path.startswith(("lib/auth/", "lib/rate-limit/")) or path == "lib/audit.ts": return "auth"
...
DESC = {
 "proxy.ts":"نگهبان لبه. هر درخواستی — چه صفحه چه API — پیش از هر چیز از اینجا می‌گذرد: ربات‌ها، درخواست بدون نشست به بخش مدیریت، پیوند داغ به عکس‌ها، و نشانی‌های قدیمی پنل (?tab=) همین‌جا رسیدگی می‌شوند.",
 "lib/http/route.ts":"خط لولهٔ هر مسیر API. ترتیب «نشست ← سقف نرخ ← نقش ← اعتبارسنجی Zod ← منطق» یک بار اینجا نوشته شده و هر مسیر فقط اعلام می‌کند چه می‌خواهد؛ پس هیچ مسیری نمی‌تواند ترتیب را جابه‌جا کند. خطاهای دامنه هم همین‌جا به کد وضعیت HTTP ترجمه می‌شوند.",
 "lib/http/responses.ts":"دو پاسخی که هیچ مسیری نباید خودش بسازد: خطای ۵۰۰ با کد پیگیری، و ۴۲۹ با زمان انتظار فارسی و ردِ ممیزی.",
 "lib/http/zod-messages.ts":"پیام‌های پیش‌فرض فارسی برای Zod، تا هیچ خطای اعتبارسنجی‌ای به انگلیسی به کاربر نرسد.",
 "lib/http/params.ts":"اعتبارسنجی مشترک شناسه‌ها، نامک‌ها و صفحه‌بندی در نشانی.",
 "lib/domain/errors.ts":"خطای دامنه: قاعده‌ای که شکسته شده، با پیام فارسی برای کاربر. لایهٔ داده آن را پرتاب می‌کند و خط لوله به ۴۰۰/۴۰۴/۴۰۹ ترجمه‌اش می‌کند.",
 "lib/domain/listing.ts":"قاعده‌های سفر یک آگهی: وضعیت شروع بر اساس پلن، اثر هر تصمیم کارشناس، و شکل ورودی فرم. بدون هیچ ورودی/خروجی، پس جدا تست می‌شود.",
 "lib/domain/billboard.ts":"شکل ستون‌های JSON رسانه (ترافیک، فهرست عکس‌ها و …) و فهرست وضعیت‌های قابل تصمیم.",
 "lib/domain/roles.ts":"نردبان نقش کارکنان. مشتری روی این نردبان نیست؛ نوع دیگری از حساب است.",
 "lib/domain/pricing.ts":"قیمت هفتگی، فصلی و سالانه از قیمت ماهانه — یک جا، به‌جای سه نسخهٔ ناهمگام.",
 "lib/domain/rating.ts":"گرد کردن میانگین امتیاز.",
 "lib/db/client.ts":"تنها جایی که اتصال به پایگاه داده ساخته می‌شود. قاعدهٔ ESLint وارد کردنش را بیرون از lib/db ممنوع کرده است.",
 "lib/db/billboards/index.ts":"درِ ورودیِ دادهٔ رسانه‌ها. هر مصرف‌کننده از همین یک نشانی می‌خواند و تقسیم‌بندی درونی به بیرون نشت نمی‌کند.",
 "lib/db/billboards/queries.ts":"هر خواندنِ جدول رسانه‌ها: پالایش و صفحه‌بندی، جست‌وجوی شعاعی، نقاط نقشه، جدول مدیریت و پیشنهادهای مرتبط. هیچ‌چیز در آن نمی‌نویسد.",
 "lib/db/billboards/mutations.ts":"هر نوشتنِ جدول رسانه‌ها از سمت مدیریت: ساخت، ویرایش، عکس‌ها و حذف با سنگِ‌قبر. هر نوشتنی که دیدِ بازدیدکننده را عوض کند، برچسب کش را باطل می‌کند.",
 "lib/db/billboards/core.ts":"واژگانِ مشترکِ دو نیمه: نگاشت ردیف به رکورد با اعتبارسنجی ستون‌های JSON، دو باریک‌سازیِ بیرونی، برچسب کش و تعریف «منتشرشده».",
 "lib/db/listings.ts":"آگهی‌های کاربران از ثبت تا انتشار. هر تغییر وضعیت یک به‌روزرسانی شرطی است، پس کلیک دوباره یا دو زبانهٔ مدیر نمی‌توانند هر دو بنویسند.",
 "lib/db/reviews.ts":"نظرها و پاسخ‌ها. نوشتن نظر و بازمحاسبهٔ میانگین امتیاز در یک تراکنش انجام می‌شوند.",
 "lib/db/leads.ts":"سرنخ‌ها: هر درخواست شماره یک ردیف یکتا برای (رسانه، مشتری)، با شمارنده‌ای که اتمی بالا می‌رود.",
 "lib/db/customers.ts":"حساب مشتری‌ها: ثبت‌نام با کد تأیید، ورود، ویرایش پروفایل و فهرست مدیریتی.",
 "lib/db/staff.ts":"حساب کارکنان، جدا از مشتری‌ها — مثل بیشتر محصولات. تغییر نقش یا غیرفعال کردنِ حساب خود ممنوع است.",
 "lib/db/idempotency.ts":"کلید یکتایی عملیات. اگر همان درخواست دوباره برسد، پاسخ ذخیره‌شده برمی‌گردد و رکورد دوم ساخته نمی‌شود.",
 "lib/db/otp-codes.ts":"رمز یک‌بارمصرف. فقط درهمِ کد ذخیره می‌شود، نه خود کد.",
 "lib/db/audit-log.ts":"ردیف‌های ماندگار ممیزی و پاکسازی فرصت‌طلبانهٔ ردیف‌های قدیمی‌تر از ۹۰ روز.",
 "lib/db/cached.ts":"لایهٔ کش روی همان تابع‌های داده؛ نه نسخهٔ دوم آن‌ها.",
 "lib/auth/session.ts":"ساخت و خواندن توکن نشست. توکن می‌گوید حساب از کدام نوع است (مشتری یا کارمند) و ادعاهایش هنگام خواندن اعتبارسنجی می‌شود.",
 "lib/auth/actor.ts":"«چه کسی درخواست می‌دهد» — نوع‌دار. توکن کارمند با ردیف پایگاه داده تطبیق داده می‌شود تا حساب غیرفعال‌شده فوراً دسترسی‌اش را از دست بدهد.",
 "lib/auth/passwords.ts":"درهم‌سازی و مقایسهٔ گذرواژه با bcrypt؛ برای حساب ناموجود هم یک مقایسهٔ ساختگی انجام می‌شود تا زمان پاسخ لو ندهد کدام شماره ثبت است.",
 "lib/auth/client-ip.ts":"نشانی شبکهٔ درخواست‌دهنده. سرآیندهای قابل جعل به‌تنهایی مبنا نیستند.",
 "lib/rate-limit/index.ts":"همهٔ سقف‌های نرخ پروژه با نام، در یک فایل.",
 "lib/rate-limit/stores.ts":"جای شمارنده‌ها: حافظه برای یک پردازه، Redis (با یک اسکریپت اتمی) وقتی REDIS_URL تنظیم باشد — همان کلیدی که کش مشترک را روشن می‌کند.",
 "lib/audit.ts":"رد ممیزی: خط لاگ و حافظهٔ کوتاه برای هر رخداد، ردیف ماندگار برای کارهایی که باید بعداً پاسخ‌گو بود.",
 "lib/env.ts":"اعتبارسنجی متغیرهای محیطی هنگام بالا آمدن. اگر تنظیمی کم باشد برنامه همان‌جا می‌ایستد.",
 "lib/types.ts":"انواع دادهٔ دامنه و برچسب‌های فارسی، بدون هیچ داده‌ای، تا صفحه‌ها با وارد کردن یک نوع، کل مجموعهٔ داده را به مرورگر نفرستند.",
 "lib/data.ts":"مجموعهٔ دادهٔ ایستا و گردآوری‌شده. فقط prisma/seed.ts آن را وارد می‌کند و قاعدهٔ ESLint بقیه را منع کرده است.",
 "lib/uploads.ts":"پذیرش فایل از کاربر. نوع فایل از بایت‌های آغازین خودش تشخیص داده می‌شود، نه از ادعای مرورگر.",
 "lib/sms.ts":"لایهٔ پیامک. بدون کلید سرویس در حالت خواب می‌ماند.",
 "lib/logger.ts":"ثبت ساخت‌یافته: هر رخداد یک شیء JSON در یک خط.",
 "lib/site-url.ts":"یک منبع واحد برای نشانی سایت.",
 "lib/client/clipboard.ts":"نسخه‌برداری متن با مسیر جایگزین، چون واسط استاندارد مرورگر بیرون از بستر امن وجود ندارد.",
 "lib/client/fetch-json.ts":"هر درخواست مرورگر با مهلت زمانی و پیام خطای فارسی، تا دکمه هرگز برای همیشه در حال ارسال نماند.",
 "lib/client/use-current-user.tsx":"قلاب سمت مرورگر برای دانستن اینکه کاربر وارد شده یا نه.",
 "lib/client/theme.tsx":"حالت روشن و تاریک.",
 "lib/format.ts":"قالب‌بندی عدد و قیمت به فارسی.",
 "lib/geo/iran-cities.ts":"فهرست استان‌ها و شهرهای ایران برای پالایه‌ها.",
 "lib/geo/distance.ts":"فاصله و مختصات برای جست‌وجوی شعاعی و نقشه.",
 "app/layout.tsx":"چیدمان ریشه. جهت راست‌به‌چپ، فونت و پوستهٔ مشترک همهٔ صفحه‌ها.",
 "app/page.tsx":"صفحهٔ نخست؛ مؤلفهٔ سمت سرور. آمار و رسانه‌های شاخص را مستقیم از لایهٔ داده (با کش) می‌خواند.",
 "app/explore/page.tsx":"صفحهٔ جست‌وجو و پالایش. نشانیِ صفحه منبع حقیقت پالایه‌هاست و سرور با همان پرس‌وجو می‌کند.",
 "app/billboard/[slug]/page.tsx":"صفحهٔ هر رسانه؛ مؤلفهٔ سمت سرور که مستقیم لایهٔ داده را صدا می‌زند و HTML آماده می‌فرستد.",
 "app/dashboard/page.tsx":"پیشخوان مشتری: حساب و آگهی‌هایش روی سرور خوانده می‌شوند و بخش تعاملی در مرورگر اجرا می‌شود.",
 "app/list-media/page.tsx":"فرم چندمرحله‌ای ثبت آگهی با بارگذاری تصویر.",
 "app/compare/page.tsx":"مقایسهٔ کنار هم چند رسانه.",
 "app/admin/(panel)/layout.tsx":"قاب پنل مدیریت. حساب کارمند پیش از رندر روی سرور بررسی می‌شود؛ هر بخش پنل نشانی خودش را دارد.",
 "app/admin/(panel)/page.tsx":"نمای کلی پنل: شمارنده‌ها مستقیم از لایهٔ داده روی سرور.",
 "app/global-error.tsx":"آخرین تور ایمنی. اگر همه چیز بشکند، کاربر به‌جای صفحهٔ سفید یک پیام فارسی با شناسهٔ پیگیری می‌بیند.",
 "app/error.tsx":"مرز خطا برای صفحه‌ها.",
 "app/not-found.tsx":"صفحهٔ ۴۰۴ فارسی.",
 "app/api/billboards/route.ts":"فهرست رسانه‌ها با پالایش، مرتب‌سازی و صفحه‌بندی؛ هر مقدار پیش از رسیدن به پرس‌وجو با فهرست مجاز سنجیده می‌شود.",
 "app/api/billboards/[slug]/contact/route.ts":"دریافت شمارهٔ تماس مالک. عمداً POST است: هم سرنخ می‌سازد، هم برداشت انبوه را ناممکن می‌کند.",
 "app/api/auth/login/route.ts":"ورود مشتری و کارمند از یک فرم. پیام خطا برای شمارهٔ ناموجود و گذرواژهٔ اشتباه یکسان است.",
 "app/api/listings/route.ts":"ثبت آگهی تازه با کلید یکتایی.",
 "app/api/admin/listings/[id]/decision/route.ts":"تصمیم کارشناس دربارهٔ یک آگهی. تک‌شلیک است: تلاش دوم پاسخ تعارض می‌گیرد.",
 "app/api/reviews/route.ts":"خواندن و ثبت نظر.",
 "prisma/seed.ts":"پرکردن اولیهٔ پایگاه داده. تنها فایلی که lib/data.ts را وارد می‌کند.",
 "prisma/sync-scraped.ts":"ورود شبانهٔ دادهٔ خزنده با ادغام سه‌طرفه، تا هیچ ویرایش یا تصمیم مدیر بازنویسی نشود.",
 ...
 "components/TrafficMeter.tsx":"نمایش تخمین بازدید روزانه.",
 ...
 "components/admin/AdminShell.tsx":"قاب پنل مدیریت: نوار بالا و منوی بخش‌ها به‌صورت پیوند.",
 "components/admin/constants.ts":"برچسب‌ها و رنگ‌های مشترک پنل مدیریت.",
...
HUBS = {"lib/api-log.ts":"api-log", "lib/auth/session.ts":"session",
        ...
        "lib/types.ts":"types", "lib/db/billboards/index.ts":"db/billboards",
...
def build_graph_svg():
    ...
    کشیدنِ هر ۱۱۶ فایلِ متصل و ۴۳۳ یال، شکل را به یک کلاف تبدیل می‌کرد (۳۴ فایلِ
