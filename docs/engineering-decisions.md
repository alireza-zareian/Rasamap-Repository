...
*Recounted 2026-09-26:* 203 files, 526 edges, 174 connected, still zero cycles
...
## 35. An adversarial architecture review, and what it changed

Every earlier review of this codebase read the documentation first and then the
code through it. This one was asked to do the opposite: assume the structure
grew by accretion rather than by design, and find what is wrong at the root.
It found nine things. One was deliberately left alone (below); the other eight
are now fixed, and each is enforced by the toolchain rather than by a paragraph.

### What was wrong, and what replaced it

**1. The security order lived in prose.** "Session → rate limit → Zod →
logic" was typed out by hand in 35 route handlers, and the only guard was a
test that grepped admin routes for the word `RateLimit`.
→ `lib/http/route.ts`: every route is `defineRoute(spec, handler)`. The spec
says who may call it (`access`), which limit applies (`rateLimit`, required by
the type), and the shape of its params, query and body; the pipeline runs them
in the one order, maps a `DomainError` to its status, writes the request log,
and marks non-public responses `no-store`. A guard test now fails if any route
exports a handler around it. This is the "declare the contract, let the
framework run it" shape of tRPC procedures and NestJS guards/pipes, in about
250 lines and no dependency.

**2. There was no data layer — only a billboards layer.** 24 of the 35 route
files queried Prisma directly; business rules (the rating rollup, the listing
state machine, the lead get-or-create) lived inside HTTP handlers, the rating
rollup twice.
→ `lib/db/` is now the Data Access Layer the Next.js data-security guide
recommends (`node_modules/next/dist/docs/01-app/02-guides/data-security.md`):
`server-only`, one module per resource, authorisation on a resource checked
there (the actor is passed in), minimal objects returned, refusals thrown as
`DomainError`s with Persian messages. Rules with no I/O moved to `lib/domain/`
and are unit-tested with no build or database (`npm run test:unit`, half a
second). ESLint's `no-restricted-imports` refuses `@/lib/db/client` outside
`lib/db/`, `@/lib/data` everywhere but the seed, and any I/O import in
`lib/domain/`. Files importing the database client: 30 → 13, all in `lib/db/`.

**3. The session did not say whose it was.** Customers (`users`) and staff
(`admins`) are two tables with overlapping ids, and the token carried a bare
`userId` plus a `role` in which "user" meant customer. A caller that forgot to
decode the table from the role wrote a staff id into a customer foreign key —
which is exactly what `POST /api/listings` did for a staff session, attributing
the listing to whichever customer shared the number.
→ The token carries `kind: "customer" | "staff"` and is validated with Zod on
read; `lib/auth/actor.ts` turns it into a discriminated `Actor`, re-reading a
staff row on every request. A route declared `access: "customer"` is handed a
`CustomerActor` by type. Two tables stay — separating workforce from customer
identity is the ordinary choice (Shopify staff accounts, Stripe dashboard
users) — but every reference to either is now a real foreign key:
`review_replies.staffId`, `audit_logs.adminId` (which had always been written
as NULL) and `audit_logs.userId`. The `owners` table, which no write path
populated, is gone.

**4. `billboards.status` answered two questions.** Whether the board is free
(available / busy / reserved / inactive) and whether the listing had passed
review (pending / awaiting_payment / needs_revision / rejected) shared one text
column. Every public query had to carry a list of four statuses to exclude;
the admin edit form could set a listing to `pending` and step around the review
state machine; the landing page's daily reach and the /about counts each
excluded a *different* subset.
→ Two enum columns, `availability` and `moderation`. "Published" is
`moderation = approved`, spelled once (`published` in `core.ts`). The edit
form can change availability only; moderation moves only through the decision
route. The crawler's bookkeeping (url, structureCode, scrapedAt, the sync
snapshot, missingSince) moved to `billboard_sources`, one row per crawled
billboard. `icon`, `mapX` and `mapY`, which nothing read, were dropped. Every
closed set is a Prisma enum (type, availability, moderation, plan, lead status,
staff role, OTP purpose, audit severity, tombstone reason).

The migration (`20260925090000_split_billboard_state`) was generated by
`prisma migrate diff` and edited where marked, because SQLite rebuilds a table
to drop a column: the data moves have to run while the old table still exists,
and the partial unique index from §8 has to be recreated on the new one. It was
applied to a copy of the real database first — 3,536 rows before and after, 4
in review, 3,511 source records, `integrity_check` and `foreign_key_check`
clean, and `migrate diff` against the schema empty — and only then to `dev.db`
(backup in `backups/`).

**5. JSON columns were trusted.** `fromRow` asserted `traffic`, `images` and
the other JSON columns with `as unknown as`.
→ They are read through Zod schemas (`lib/domain/billboard.ts`). A malformed
value is logged with its row and column and replaced by its empty shape — one
bad row written by a script costs that row its feature list, not a catalogue
page a 500.

**6. The pages split by accident, not by rule.** Public pages rendered on the
server; everything behind a sign-in was a client component that rendered a
spinner and fetched its first screen. The admin panel was one 400-line client
component holding eight tabs in state.
→ One rule for every page: the first screen is rendered with its data on the
server; the browser asks the API only in response to what the user does. The
panel is `app/admin/(panel)/`: a server layout that checks the staff account
before rendering, one route per section (bookmarkable, code-split, each with
the section's own error boundary), old `?tab=` links forwarded by `proxy.ts`.
The dashboard and /analytics read their first screen on the server too.
`lib/` is grouped by where code runs: `lib/client/` (browser-only helpers),
`lib/geo/`, `lib/http/`, `lib/rate-limit/`, `lib/domain/`, `lib/db/`.

**7. The dormant shared store was half a store.** `REDIS_URL` moved the
response cache to Redis but left every rate-limit counter in the process, so
three instances meant three times the password guesses before a lockout.
→ `lib/rate-limit/stores.ts`: the same fixed-window-with-lockout algorithm in
TypeScript (memory) and as one atomic Lua script (Redis), selected by the same
`REDIS_URL`. While Redis is unreachable each call falls back to memory at once
— it never waits on the connection — so an outage degrades to the old
behaviour instead of a hung sign-in. The suite passes with no Redis, with Redis
up (counters verified in Redis), and with `REDIS_URL` pointing at a dead port.
Nothing is deployed on Redis; the switch simply works when it is turned on.

**8. Invariants were documentation.** The rules that mattered most were
paragraphs in AGENTS.md, and comments had gone stale (`audit_logs.adminId`
claimed the bootstrap admin had no row, long after the seed created one).
→ The five boundaries above are lint rules or types; `CLAUDE.md` and
`AGENTS.md` point to them instead of restating them.

### Found on the way

Behaviour that was wrong and is now tested: a limit without a lockout wrote a
durable audit row for *every* rejected request (not only the first); the admin
photo route stored any string beginning with `/` or `http` as an existing photo;
the listing edit schema had lost the trimming the duplicate index relies on;
`/about` counted rejected and unpaid submissions; the customer dashboard counted
a published but busy listing as unpublished; the compare table called every
non-free board "busy"; `scripts/to-postgres.mjs` never copied
`source_tombstones`; Zod's English default messages reached Persian forms.

### What was deliberately not changed

The crawler invents a rating, a review count and, when a listing omits them,
dimensions (`scraper/scraper.py`), and 3,530 of 3,536 rows carry those numbers
while five real reviews exist. The review flagged this as the most serious
finding — a marketplace showing invented reviews. It is kept, knowingly, for
the demo; the first real review of a row replaces its invented rating with the
real average (lib/db/reviews.ts). Before any real launch the columns should be
nullable and the invented values removed.

### Where things moved

| Was | Is |
|---|---|
| `lib/api-log.ts` (`withApiLog`), `lib/api-error.ts`, `lib/api-rate-limit.ts` | `lib/http/route.ts` (`defineRoute`), `lib/http/responses.ts` |
| `lib/auth/rate-limit.ts` | `lib/rate-limit/index.ts` (policies), `lib/rate-limit/stores.ts` |
| `lib/auth/users.ts` (`getStaffSession`, `hasPermission`) | `lib/auth/actor.ts` (`getActor`, `requireStaff`), `lib/domain/roles.ts` (`hasRole`), `lib/db/staff.ts`, `lib/auth/passwords.ts` |
| `lib/auth/audit.ts` (`persistAudit`) | `lib/audit.ts` (`recordAudit`), `lib/db/audit-log.ts` |
| `lib/idempotency.ts`, `lib/otp.ts` | `lib/db/idempotency.ts`, `lib/db/otp-codes.ts` |
| `lib/clipboard.ts`, `lib/fetch-json.ts`, `lib/theme.tsx`, `lib/useModalA11y.ts`, `lib/auth/useCurrentUser.tsx` | `lib/client/` (kebab-case) |
| `lib/geo.ts`, `lib/iranLocations.ts`, `lib/iran-provinces.ts` | `lib/geo/distance.ts`, `lib/geo/iran-cities.ts`, `lib/geo/iran-provinces.ts` |
| `app/admin/page.tsx` | `app/admin/(panel)/*` |
| `billboards.status` | `billboards.availability` + `billboards.moderation` |
| `billboards.url`, `structureCode`, `scrapedAt`, `sourceSnapshot`, `missingSince` | `billboard_sources` |

### Verified

`tsc`, ESLint (with the new boundary rules, each checked against a file that
breaks it), a production build, 9 unit + 142 API + 6 importer tests, and 10
browser flows — including a reviewer signing in and moving between panel
