...
- JWT با الگوریتمِ HS256 در کوکیِ HttpOnly و SameSite=Lax، که با هر درخواست با ردیفِ حساب
  سنجیده می‌شود (تغییر یا بازنشانیِ رمز نشست‌های قبلی را می‌بندد، خروج همان توکن را باطل
  می‌کند، یک ورود حداکثر هفت روز)؛ `bcrypt` با هزینه‌ی
...
> |---|---|
> | `npm run dev` | **9.7 s** |
> | `npm run demo` (`next build && node server.mjs`) | **0.1 s** |
>
> ### ~97× less CPU.
>
...
6. **Numbers to have ready:** 3536 billboards (3532 published) across 101 cities,
   44 API endpoints in 35 route files, 179 automated tests plus 10 browser flows,
...
> | `npm run demo` = `next build && node server.mjs` | **۰.۱ ثانیه** |
>
...
| **Observability** | A− | One JSON object per log line, one `api_request` per request from the route pipeline (`lib/http/route.ts`), audit lines routed through the same logger, optional rotated file via `LOG_DIR`. §7 + §7a explain the deliberate stop point and the path forward. | No dashboards/alerting — that's the deployment layer, and the format is built for it, but it isn't wired. |
| **Testing** | A− | 179 dependency-free tests (`node:test` + `fetch` against a real **production** build on an isolated DB): 12 unit tests of the pure rules, 160 API — validation, allowlists, rate limits, no enumeration **by body or by timing**, upload magic-byte rejection, the approval state machine, object-level authz, the OTP reset flow, sort correctness (each guarded against a vacuous pass) — plus 7 covering the nightly importer's rule that it may never undo a person (§33). On top of that, 10 E2E flows drive the installed Chrome over CDP (§31). `npm run bench` for load. | No component tests, and the browser suite covers the main flows rather than every page. Reasonable for the scope and timeline; worth naming as future work. |
| **Code quality** | A− | Consistent structure across 35 route files (44 endpoints), single-responsibility modules, `0` lint warnings, no `TODO`/`FIXME`/`@ts-ignore` in the codebase, TypeScript strict. Inline-style rule inflates line counts but that's a deliberate design-system choice. | `page.tsx` files are large (600+ lines) because of inline styles; the admin billboards list was loading all rows and filtering in JS until this pass (now DB-side); one O(n²) stat was replaced with O(n). |
| **Documentation** | A | `docs/` carries architecture, a 44-endpoint API reference, 35 decision records with a milestone log, a security audit, a production-readiness triage, demo-account sheet, and this prep checklist. README is a readable narrative, not a command dump. | — |
