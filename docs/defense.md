...
6. **Numbers to have ready:** 3536 billboards (3532 published) across 101 cities,
   44 API endpoints in 35 route files, 181 automated tests plus 11 browser flows,
...
| **Testing** | A− | 181 dependency-free tests (`node:test` + `fetch` against a real **production** build on an isolated DB): 12 unit tests of the pure rules, 162 API — validation, allowlists, rate limits, no enumeration **by body or by timing**, upload magic-byte rejection, the approval state machine, object-level authz, the OTP reset flow, sort correctness (each guarded against a vacuous pass) — plus 7 covering the nightly importer's rule that it may never undo a person (§33). On top of that, 11 E2E flows drive the installed Chrome over CDP (§31). `npm run bench` for load. | No component tests, and the browser suite covers the main flows rather than every page. Reasonable for the scope and timeline; worth naming as future work. |
