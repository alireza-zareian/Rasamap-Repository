...
رساماپ یک بازارگاهِ آنلاین برای رزرو رسانه‌های تبلیغاتی محیطی (بیلبورد) است. کاربر
بین حدود ۳۵۰۰ رسانه جست‌وجو و فیلتر می‌کند، صفحه‌ی جزئیات را می‌بیند، و پس از
ثبت‌نام با شماره‌ی موبایل یک بازه‌ی زمانی را رزرو می‌کند. یک پنلِ مدیریتیِ جدا هم
برای رسیدگی به رسانه‌ها و رزروها هست. رابطِ کاربری کاملاً فارسی و راست‌به‌چپ است.
...
1. **مرورگر به API.** همه‌ی صفحه‌های کلاینتی با `fetch("/api/...")` کار می‌کنند،
   چون پس از بارگذاری به داده‌ی تازه و تعاملی نیاز دارند — فیلتر، صفحه‌بندی، رزرو.
...
- نقش‌ها: `viewer < editor < admin < super_admin` به‌علاوه‌ی `user`. مرزِ امنیت
  ...
  می‌گیرد و کاربر نمی‌تواند رزروِ کاربرِ دیگر را ببیند.
...
- **Idempotency-Key** (هدرِ اختیاری) روی رزرو و ثبتِ رسانه: کلیدِ تکراری همان
  پاسخِ قبلی را برمی‌گرداند، نه رکوردِ دوم. + قیدِ unique در پایگاه داده روی
  `(بیلبورد، کاربر، تاریخِ شروع، تاریخِ پایان)`.
- رزرو: بررسیِ هم‌پوشانی و درج داخلِ یک تراکنش. آزمونِ هم‌زمانی: **ده درخواستِ
  یکسان و هم‌زمان، دقیقاً یک رزرو می‌سازد** و نُه تای دیگر ۴۰۹ می‌گیرند.
...
- ثبتِ ممیزیِ پایدار برای کارهای مدیر (ساخت/ویرایش/حذفِ رسانه، تغییرِ وضعیتِ رزرو)
...
- سوییتِ آزمونِ بدونِ وابستگی (`npm test`): `node:test` داخلیِ Node در برابرِ یک
  سرورِ واقعیِ Next روی یک پایگاه داده‌ی تستِ جدا — نه `dev.db`. ۲۷ آزمون:
  اعتبارسنجی، allowlistها، محدودیتِ نرخ، عدمِ نشتِ نامِ کاربری، گاردِ هم‌زمانیِ
  رزرو، دسترسیِ سطحِ رکورد، RBAC، و ممیزیِ پایدار.
...
- **نوشتنِ هم‌زمان:** SQLite در لحظه یک نویسنده دارد. زیرِ بارِ سنگینِ رزرو،
  `POST /api/reservations` نخستین جایی است که کند می‌شود. مسیرِ ارتقا: Postgres.
...
> «رساماپ یک بازارگاهِ رزرو بیلبورد است که با Next.js ساخته شده — یک پایگاه کد که
...
> خطا، محدودیتِ نرخ، هم‌زمانیِ رزرو و بازیابی همه پیاده و آزموده شده‌اند؛ چیزهایی
