...
> `status` به `availability` و `moderation` شکسته شد و وضعیت خزنده به `billboard_sources`
> رفت؛ نشست نوع حساب (مشتری/کارمند) را با خودش دارد؛ پنل مدیریت مسیرهای جدا با بررسی
> سمت سرور دارد. نشانی‌های قدیمی فایل‌ها در جدول «Where things moved» همان بخش آمده است.
> **باقی‌مانده، آگاهانه:** امتیاز و تعداد نظرِ ساختگیِ ردیف‌های خزنده برای دمو نگه داشته
> شده؛ پیش از راه‌اندازی واقعی باید حذف شود.
...
- [x] **MAP-B** — **درست بود.** اندازه‌گیری روی دیتاست: از ۳٬۰۳۲ رکوردِ مختصات‌دار، حدود
      ...
      تعمیر**: `isPlottable()` در `lib/geo/distance.ts` هر نقطهٔ دورتر از ۴۰ کیلومتر از مرکزِ
...
**فایل‌ها:** `app/explore/map/page.tsx` · `components/IranMap.tsx` · `lib/geo/distance.ts` ·
`lib/geo/iran-provinces.ts` (تولیدشده، ۴۲KB) · `scripts/build-iran-map.py`.
