...
const SCRAPER_SOURCES = [
  { key: "billboardiha", label: "Billboardiha.com", icon: "🗺" },
  { key: "geocoding",    label: "Geocoding فقط",   icon: "🌐" },
  { key: "cache_repair", label: "ترمیم کش",        icon: "🔧" },
];
...
export function ScraperPanel({ canRun }: { canRun: boolean }) {
  ...
  return (
    <div>
      <div style={{ fontSize: "0.9rem", fontWeight: 700, marginBottom: 6 }}>🤖 مدیریت اسکرپر</div>
          ...
          return (
            <div key={src.key} style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <div style={{ fontWeight: 600, fontSize: "0.85rem" }}>{src.icon} {src.label}</div>
                <Badge
                  text={status === "idle" ? "آماده" : status === "running" ? "در حال اجرا" : status === "done" ? `✓ ${run?.count ?? ""}` : "خطا"}
